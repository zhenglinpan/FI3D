...
class Text2MotionDataset(data.Dataset):
    def __init__(self, opt, mean, std, split_file, w_vectorizer):
        ...
        for name in tqdm(id_list):
            try:
                motion = np.load(pjoin(opt.motion_dir, name + '.npy'))  # (seq_len, 263)
                if (len(motion)) < min_motion_len or (len(motion) >= 200):
                    continue
                text_data = []
                flag = False
                with cs.open(pjoin(opt.text_dir, name + '.txt')) as f:  # ./dataset/HumanML3D/texts/000000.txt
                    for line in f.readlines():
                        text_dict = {}
...
class Text2MotionDatasetV2(data.Dataset):
    def __init__(self, opt, mean, std, split_file, w_vectorizer):
        ...
        for name in tqdm(id_list):
            try:
                motion = np.load(pjoin(opt.motion_dir, name + '.npy'))  # motion loaded here is (seq_len, 263)
                # print("file name", pjoin(opt.motion_dir, name + '.npy'))
                if (len(motion)) < min_motion_len or (len(motion) >= 200):
                    continue
                text_data = []
                flag = False
    ...
    def __getitem__(self, item):
        ...
        unpadded_motion = motion.copy() # ! BUG: unpadded motion 由于长度不一致(seq, 263)导致dataloader读取出错？那之前怎么没问题？之前直接走的get_co_embeddings, 可能没到这里
        if m_length < self.max_motion_length:   # <<< pad the motion to 196, max_motion_length 
            motion = np.concatenate([motion,    # ! 可以用 pack_padded_sequence处理这种问题，不过这里我先去看get_co_embeddings了
                                     np.zeros((self.max_motion_length - m_length, motion.shape[1]))
                                     ], axis=0)
        # print(word_embeddings.shape, motion.shape)
        # print(tokens)
        
        return word_embeddings, pos_one_hots, caption, sent_len, motion, unpadded_motion, m_length, '_'.join(tokens)
...
class RawTextDataset(data.Dataset):
    def __init__(self, opt, mean, std, text_file, w_vectorizer):
        ...
        with cs.open(text_file) as f:
            for line in f.readlines():
                word_list, pos_list = self.process_text(line.strip())   # A man is walking
                tokens = ['%s/%s'%(word_list[i], pos_list[i]) for i in range(len(word_list))]   # A/NN man/NN is/VBZ walking/VBG
                self.data_dict.append({'caption':line.strip(), "tokens":tokens})    # 'A man is walking': A/NN man/NN is/VBZ walking/VBG
    ...
    def __getitem__(self, item):
        ...
        if len(tokens) < self.opt.max_text_len:
            # pad with "unk", unknowns like 'sos' or 'eos'
            tokens = ['sos/OTHER'] + tokens + ['eos/OTHER']
            sent_len = len(tokens)
            tokens = tokens + ['unk/OTHER'] * (self.opt.max_text_len + 2 - sent_len)    # sos/OTHER A/NN man/NN is/VBZ walking/VBG eos/OTHER unk/OTHER unk/OTHER unk/OTHER..
        else:
            # crop
            tokens = tokens[:self.opt.max_text_len]
            tokens = ['sos/OTHER'] + tokens + ['eos/OTHER']
        ...
        word_embeddings = []
        for token in tokens:
            word_emb, pos_oh = self.w_vectorizer[token] # vectorize the word and pos separately, token looks like 'man/NN'
            pos_one_hots.append(pos_oh[None, :])
            word_embeddings.append(word_emb[None, :])
        pos_one_hots = np.concatenate(pos_one_hots, axis=0)
        word_embeddings = np.concatenate(word_embeddings, axis=0)
