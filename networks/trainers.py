...
class CompTrainerV6(object):
    ...
    def generate(self, word_emb, pos_ohot, cap_lens, m_lens, mov_len, dim_pose):
        ...
        for i in range(mov_len):
            # print("\t Sequence Measure")
            # print(mov_in.shape)   # torch.Size([1, 512]), in a loop of mov_len
            '''Local Attention Vector'''
            ...
            '''Decoder'''
            dec_in = torch.cat([mov_in, att_vec, z_pri], dim=-1)    # mov_in is the encoder output, plays as the input for seq_dec, seq_dec is TextVAEDecoder()
    ...
    def backward_G(self):
        ...
        # print('self.motions\n', self.motions.shape) #  torch.Size([32, 40, 263])
        self.loss_mov_rec = self.l1_criterion(self.fake_movements, self.movements)
        # print('self.movements\n', self.movements.shape) #  torch.Size([32, 10, 512])
...
class MotionGruTrainer(object):
    def __init__(self, args, text_encoder, motion_encoder, motion_decoder, movement_encoder):
        self.movement_encoder = movement_encoder
        self.motion_encoder = motion_encoder
    ...
    def to(self, device=torch.device(0)):
        self.motion_encoder.to(device)
    ...
    def forward(self, batch_data):
        _, _, _, _, motions, m_lens, _ = batch_data
        motions = motions.detach().to(self.device).float()
        ...
        '''Motion Decoding'''
        self.recon_motions = self.motion_decoder(self.motion_embedding, m_lens)
        

    def backward(self):
        batch_size = self.motion_embedding.shape[0]
        
        self.loss_rec = self.l1_criterion(self.recon_motions, self.motion_embedding)
        self.loss_sparsity = torch.mean(torch.abs(self.latents))
        self.loss_smooth = self.l1_criterion(self.latents[:, 1:], self.latents[:, :-1])
        self.loss_normality = None  
        
        lambda_sparsity = 0.1
        lambda_smooth = 0.1
        lambda_normality = 0.1
        self.loss = self.loss_rec \
                    + self.loss_sparsity * lambda_sparsity \
                    + self.loss_smooth * lambda_smooth \
                    + self.loss_normality * lambda_normality
        
        loss_logs = OrderedDict({})
        loss_logs['loss'] = self.loss.item()
        loss_logs['loss_rec'] = self.loss_rec.item()
        loss_logs['loss_sparsity'] = self.loss_sparsity.item() * lambda_sparsity
        loss_logs['loss_smooth'] = self.loss_smooth.item() * lambda_smooth
        loss_logs['loss_normality'] = self.loss_normality.item() * lambda_normality
        
        return loss_logs
        
    def update(self,):
        self.zero_grad([self.optimizer_motion_encoder, self.optimizer_motion_decoder])
        loss_logs = self.backward()
        self.loss.backward()
        self.clip_norm([self.motion_encoder, self.motion_decoder])
        self.step([self.optimizer_motion_encoder, self.optimizer_motion_decoder])
    ...
    def train(self, train_dataloader, val_dataloader):
        ...
        self.optimizer_motion_encoder = optim.Adam(self.motion_encoder.parameters(), lr=0.0002)
        self.optimizer_motion_decoder = optim.Adam(self.motion_decoder.parameters(), lr=0.0002)
        ...
        for epc in range(epoch):
            ...
            # if epc % 2 == 0:
            #     self.save(f'models_{epc}.tar', epoch, it)
